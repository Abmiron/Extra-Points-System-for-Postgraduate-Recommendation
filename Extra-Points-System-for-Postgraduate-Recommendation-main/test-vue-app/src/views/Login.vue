...
const handleLogin = async () => {
  loading.value = true
  console.log('开始登录流程，表单数据:', loginForm)

  try {
    // 直接调用authStore的login方法，传递表单数据
    const loginSuccess = await authStore.login(loginForm)
    console.log('登录方法返回:', loginSuccess)
    
    // 等待一小段时间确保状态更新完成
    await new Promise(resolve => setTimeout(resolve, 100))
    
    // 根据用户角色跳转页面
    // 角色信息应该在authStore中已经设置
    const userRole = authStore.role
    const token = authStore.token
    const user = authStore.user
    
    console.log('登录后角色:', userRole)
    console.log('登录后令牌:', token)
    console.log('登录后用户信息:', user)
    ...
    if (userRole && routeMap[userRole]) {
      console.log('跳转到:', routeMap[userRole])
      router.push(routeMap[userRole])
    } else {
      // 默认跳转到首页或学生页面
      console.log('跳转到默认页面: /student')
      router.push('/student')
    }
  } catch (error) {
