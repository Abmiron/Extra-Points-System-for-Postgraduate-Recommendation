...
const userInfo = computed(() => ({
  ...
  roleName: authStore.user?.roleName || ''  // 移除默认的系统管理员角色
}))
...
onMounted(() => {
  ...
  const hasAdminRole = authStore.role === 'admin' && localStorageRole === 'admin'
  
  console.log('Admin组件权限验证:', {
  ...
  if (!hasAdminRole) {
    console.error('权限验证失败：不是管理员账号，清除认证信息')
    // 清除可能有误的认证信息
