...
def verify_test_user():
    ...
    with app.app_context():
        # 查询用户记录
        user = User.query.filter_by(username='teststudent004').first()
        print(f"\n=== 用户记录查询结果 ===")
        if user:
        ...
        # 查询学生记录
        student = Student.query.filter_by(student_id='teststudent004').first()
        print(f"\n=== 学生记录查询结果 ===")
        if student:
...
def cleanup():
    ...
    with app.app_context():
        ...
        # 删除之前的测试数据
        user003 = User.query.filter_by(username='teststudent003').first()
        if user003:
            db.session.delete(user003)
            print(f"已删除用户: {user003.username}")
        
        student003 = Student.query.filter_by(student_id='teststudent003').first()
        if student003:
            db.session.delete(student003)
            print(f"已删除学生: {student003.student_id}")
