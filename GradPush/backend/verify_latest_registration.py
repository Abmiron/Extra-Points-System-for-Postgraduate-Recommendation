...
with app.app_context():
    print("验证最新注册的用户和学生记录...")
    
    # 检查用户记录
    print("\n1. 用户记录 (User):")
    user = User.query.filter_by(username='teststudent002').first()
    if user:
    ...
    # 检查学生记录
    print("\n2. 学生记录 (Student):")
    student = Student.query.filter_by(student_id='teststudent002').first()
    if student:
    ...
    if students:
        for student in students:
            print(f"   学号: {student.student_id}, 姓名: {student.student_name}")
    else:
        print("   Student表中没有记录")